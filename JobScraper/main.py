...
async def scrape_jobs_endpoint(
    ...
    results_wanted: int = Query(
        ...
        ge=1,
        le=50,
        description="Max results per job title per site (e.g. 5 titles × 5 results = 25 total).",
    ),
    hours_old: int = Query(
        72,
        ge=1,
    ...
    # Convert DataFrame to JSON-safe records (NaN → None, dates → ISO strings)
